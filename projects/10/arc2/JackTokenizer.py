...
THINGS_TO_ELIMINATE = re.compile(r'([\(\)\[\]\{\}\,\;\=\.\+\-\*\/\&\|\~\<\>\s]|(?:"[^"]*"))')
...
class JackTokenizer:
    ...
    def __init__(self, input_stream: typing.TextIO) -> None:
        ...
        self.tokens_lst = self.generate_tokens()
        self.cur_token = None
        self.cur_token_num = 0

    def ignore_comments(self, current, i):
        if '*/' in current:
            return self.input_lines[i], i
        return self.ignore_comments(self.input_lines[i].strip(), i + 1)
    ...
    def generate_tokens(self):
        ...
        while i < len(self.input_lines):
            ...
            if '/**' in current:
                current, i = self.ignore_comments(self.input_lines[i].strip(), i)
            # if empty line
            if current:
                ...
                # in-line comments /* */
                while '/*' in current:
                    current, i = self.remove_in_line_comments(current, i)
        ...
        return [token for token in THINGS_TO_ELIMINATE.split(file_string) if token not in ('', ' ')]

    def remove_in_line_comments(self, line, i):
        ...
        if start == -1 and end == -1:
            return line
        if start != -1 and end == -1:
            return self.ignore_comments(line, i)
        in_line_comment = line[start: end + 2]
        line = line.replace(in_line_comment, '')
        return self.remove_in_line_comments(line, i), i
